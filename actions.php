...
for ($awm_t = 0; $awm_t < $awm_total_tabs; $awm_t++) {
	...
	if (isset($_POST['AWM_categories_subitems_no_' . $awm_t])) {
		if ($_POST['AWM_categories_subitems_no_' . $awm_t] < 1) {
			$_POST['AWM_categories_subitems_no_' . $awm_t] = 1;
		}
		if ($_POST['AWM_categories_subitems_no_' . $awm_t] > 50) {
			$_POST['AWM_categories_subitems_no_' . $awm_t] = 50;
		}
	}
	// AWM_menu_name
	if (isset($_POST["AWM_menu_name_" . $awm_t])) {
		$_POST["AWM_menu_name_" . $awm_t] = awm_fix_menu_name($_POST["AWM_menu_name_" . $awm_t]);
	}
}
// AWM_menu_path
if (isset($_POST["AWM_menu_path"])) {
	$awm_path = (string)$_POST["AWM_menu_path"];
	if ((strpos($awm_path, "/") != 0) || (strpos($awm_path, "/") === false)) {
		$awm_path = "/" . $awm_path;
	}
	if (substr($awm_path, strlen($awm_path) - 1, 1) != "/") {
		$awm_path = $awm_path . "/";
	}
...
if ($_POST['theaction'] == "createnew") {
	...
	if ($_POST['theaction'] == "delete") {
		...
		if ($_POST['theaction'] == "set_defaults") {
			...
			if ($_POST['theaction'] == "info_update") {
				// update all values
				...
				if ($_POST['theaction'] == 'hide_msg') {
					update_option('AWM_Check_show', 0);
					ob_end_clean();
					wp_redirect($_POST['ref']);
					exit;
				} else {
					if ($_POST['theaction'] == 'hide_addcode') {
						update_option("AWM_code_check", 0);
						ob_end_clean();
						wp_redirect($_POST['ref']);
						exit;
					} else {
						if ($_POST['theaction'] == "zip_update") {
							if (strpos(urldecode($_SERVER['HTTP_REFERER']),
									"wp-admin/options-general.php?page=allwebmenus-wordpress-menu-plugin/allwebmenus-wordpress-menu.php") === false) {
